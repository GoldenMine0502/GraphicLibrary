...
public class MousePicker {
    ...
    public MousePicker(Camera cam, Matrix4f projection, Palette palette) {
        camera = cam;
    ...
    public void update(float mouseX, float mouseY) {
        ...
        if (intersectionInRange(0, RAY_RANGE, currentRay)) {
            List<Sprite> sprite = palette.getSprites();
    ...
    private Vector2f getNormalisedDeviceCoordinates(float mouseX, float mouseY) {
        float x = (2.0f * mouseX) / (float) palette.getPaletteSizeX() - 1f;
        float y = (2.0f * mouseY) / (float) palette.getPaletteSizeY() - 1f;
